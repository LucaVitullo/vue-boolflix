<template>
    <div class="row justify-content-center me-0 gap-2 text-light">
        <div class="col-3 bg_lv fs-4 p-1  border-2 m-2" v-for="item in films" :key="item.id">
            ...
            <div class="overlay">
                <div class="text fs_10 ">
                    <p class="fs_10 m-1">TITOLO: {{item.title}}</p>
                    <p class="fs_10 m-1">TITOLO ORIGINALE: {{ item.original_title }}</p>
                    <div class="container-flag  d-flex justify-content-center">
                        <img class="flag d-block" :src="showFlags (item.original_language)" 
                         @error="imageNotFound"
                        >
                    </div>
                    <div class="fs-5 d-inline-block" v-for="(n, index) in 5" :key="index">
                        <font-awesome-icon
                    ...
                    <p class="fs_10  w-100 p-4 h-100">DESCRIZIONE:  {{item.overview}}</p>
